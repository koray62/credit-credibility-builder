import React, { useState } from 'react'; import Navbar from '@/components/Navbar'; import Footer from '@/components/Footer';

interface PaymentPlan { taksitNo: number; taksitTutari: number; normalAnaparaTutari: number; araOdeme: number; toplamAnaparaTutari: number; netFaizTutari: number; kkdfTutari: number; bsmvTutari: number; brutFaizTutari: number; kalanAnapara: number; tvmDegeri: number; }

interface CreditPlan { krediTutari: number; duzeltilmisKrediTutari: number; aylikNetFaizOrani: number; aylikBrutFaizOrani: number; taksitSayisi: number; taksitTutari: number; toplamOdeme: number; toplamNetFaiz: number; toplamKKDF: number; toplamBSMV: number; toplamVergiDahilFaiz: number; toplamAraOdeme: number; toplamAnapara: number; toplamIskontoTutari: number; odemePlani: PaymentPlan[]; negativeAnaparaFound: boolean; }

const CreditCalculation: React.FC = () => { const [krediTutari, setKrediTutari] = useState<number>(200000); const [aylikFaizOrani, setAylikFaizOrani] = useState<number>(3.00); const [taksitSayisi, setTaksitSayisi] = useState<number>(12); const [araOdemeler, setAraOdemeler] = useState<number[]>([]); const [globalOdemePlani, setGlobalOdemePlani] = useState<CreditPlan | null>(null); const [showResults, setShowResults] = useState<boolean>(false);

// Vergi oranları const KKDF_ORANI = 0.15; // %15 const BSMV_ORANI = 0.05; // %5

// Bugünkü Değer Hesaplama (İskonto) const bugunkuDegerHesapla = (gelecekDeger: number, aylikFaizOrani: number, donem: number): number => { return gelecekDeger / Math.pow(1 + aylikFaizOrani * (1 + KKDF_ORANI + BSMV_ORANI), donem); };

// Taksit Tutarı (Anüite) Hesaplama const taksitTutariHesapla = (krediTutari: number, aylikNetFaizOrani: number, taksitSayisi: number): number => { const brutFaizOrani = aylikNetFaizOrani * (1 + KKDF_ORANI + BSMV_ORANI); return krediTutari * (brutFaizOrani * Math.pow(1 + brutFaizOrani, taksitSayisi)) / (Math.pow(1 + brutFaizOrani, taksitSayisi) - 1); };

// Kredi Ödeme Planı Hesaplama const odemePlaniOlustur = (krediTutari: number, aylikNetFaizOrani: number, taksitSayisi: number, araOdemeler: number[] = []): CreditPlan => { let toplamIskontoTutari = 0; let tvmDegerleri = new Array(taksitSayisi).fill(0);
...
const odemePlaniTablosuOlustur = () => { if (krediTutari <= 0 || aylikFaizOrani <= 0 || taksitSayisi <= 0) { alert("Lütfen tüm alanları pozitif değerler ile doldurun."); return; }
...
const araOdemeGuncelle = (taksitNo: number, yeniDeger: string) => { const araOdemeTutari = parseFloat(yeniDeger) || 0; const newAraOdemeler = [...araOdemeler]; newAraOdemeler[taksitNo - 1] = araOdemeTutari;
...
return ( <div style={{ fontFamily: "'Segoe UI', Arial, sans-serif", lineHeight: 1.6, margin: 0, padding: 0, color: '#333', backgroundColor: '#f9f9f9' }}> <Navbar /> <div style={{ maxWidth: '1200px', margin: '30px auto', backgroundColor: '#fff', padding: '30px', borderRadius: '10px', boxShadow: '0 0 20px rgba(0,0,0,0.1)' }}> <h1 style={{ color: '#2c3e50', textAlign: 'center', marginBottom: '30px', fontSize: '32px', fontWeight: 600 }}> Kredi Ödeme Planı Hesaplama </h1>
